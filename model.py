...
class Model():
    def __init__(self, args):
     ...
     #   x_list = [tf.squeeze(x_i, [1]) for x_i in x]
        ...
        if args.mode == 'synthesis':
            ...
            for t in range(args.T):
                with tf.variable_scope("cell1", reuse=DO_SHARE):
                    # h_cell1, cell1_state = self.cell1(tf.concat([x_list[t]], 1), cell1_state)
        ...
        _concat_output_list = tf.concat(self.output_list, 1)
        _reshape_concat_output_list = tf.reshape(_concat_output_list, [-1, args.rnn_state_size])
        self.output = tf.nn.xw_plus_b(_reshape_concat_output_list, output_w, output_b)
        _reshape_y = tf.reshape(self.y, [-1, 3])
    ...
    def sample(self, sess, length, str=None):
        ...
        if self.args.mode == 'predict':
            state = sess.run(self.stacked_cell.zero_state(self.args.batch_size, tf.float32))
        ...
        for i in range(length - 1):
            if self.args.mode == 'predict':
                feed_dict = {self.x: x, self.init_state: state, self.batch_size: self.args.batch_size}
            ...
            if e < end_of_stroke:
                ...
                x[0, 0, 2] = 0
            strokes[i + 1, :] = x[0, 0, :]
        if self.args.mode == 'synthesis' and 0:
            # print kappa_list
